# landbridge-mcp runtime image (spec §3 — the per-Instance control plane container).
#
...
# src/Landbridge.Mcp so the plane project directory is never touched.
FROM mcr.microsoft.com/dotnet/aspnet:10.0 AS runtime
WORKDIR /app

# The publish output (Landbridge.Mcp.dll + deps + the embedded skill bundle).
COPY . .
...
ENTRYPOINT ["dotnet", "Landbridge.Mcp.dll"]
