...
8. **Inbox watch** — `watch_lead_inbox` / `watch_inbox` / `GET /lead/inbox/events` wake on Hub SSE, then GET. Worker `PullReceipt` is a Core write (`POST /core/v1/sessions/{id}/inbox`, or the queue when `WriteQueue` is set). Fanout when Hub is unset.
