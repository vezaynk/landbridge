...
You are an ACP session. A stop or a deliberate `park_session` arrives as `session/cancel`, then a tree-kill if you have not exited. A prose answer to a question you asked arrives as another turn on this same connection — pull it with `get_session`. There is no mid-task stdin wind-down turn.
