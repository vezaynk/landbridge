...
**Docket's tools are MCP tools, and there is no `docket` command line.** Everything this skill tells you to call — `get_task`, `report_result`, `request_input`, `start_process`, `register_service` and the rest — the harness exposes as MCP tools named `mcp__docket__get_task`, `mcp__docket__report_result`, and so on. Call them as tools, under those names. No `docket` executable exists on any machine here: the daemon is `docketd`, you never invoke it yourself, and nothing named `docket` is on any PATH. So a shell command beginning with `docket` cannot work, and neither can reaching the MCP server yourself over HTTP or with `curl` — the tool call is the only route. This is not pedantry about naming: a worker that shells out instead of calling the tool has invented a program that does not exist, and even when it guesses a plausible command it has bypassed the one path that records what it did. If a docket tool looks unavailable, or a call comes back refused, handle it the way the refusal guidance under [Asking questions](#asking-questions) says — and never by routing around it with a shell.
