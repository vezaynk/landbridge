...
**Answer input requests promptly, and answer them in words.** A worker in `blocked_on_input` occupies a machine. Permission waits stay live inside the ACP session. Prose questions may have ended the turn. Wait TTL is off by default — a forgotten question holds the lease until you answer or you `park_task`. `park_task` is the deliberate release: the session is cancelled and later redispatch is `session/load`. Parks-per-task in the Team view is the number that says whether the Team is starving on your attention.
...
Permissions are ACP `session/request_permission`. There is no bypass / always-approve flag on a Docket worker. A tool call the agent cannot take on its own comes to you as an approval request. Two things make these unlike every other blocked task:

**The worker is still running, blocked inside that tool call.** It hasn't parked and won't be redispatched — your verdict resumes it where it stands. So these are the requests to answer first: a slow answer here is a process sitting idle on a machine, not a task waiting in a queue. Wait TTL is off by default; use `park_task` if you mean to release the machine.
...
`park_task` cancels the live ACP session (`session/cancel`) and parks. Wake later is automatic when an answer lands (`session/load`).
...
`session/cancel` is delivered; cooperation is not guaranteed. What you get back is whatever it had already reported, plus a session id the plane can `session/load`. If you need to know where a long task stands, ask while it is still working rather than expecting the cancel to elicit a report.
