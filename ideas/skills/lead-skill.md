...
The same rules as any forward apply: only services registered by a currently-working task in your Team, and the service disappears when its task is accepted, failed, parked, or cancelled — a report keeps it so you can still reach it while adjudicating. If the forward fails, check `get_team_state` for whether the owning task is still live before assuming a network problem.
