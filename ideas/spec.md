...
| `services` | Operator-declared long-lived processes `docketd` supervises: `name`, `spawn` argv, `working_directory`, `env`, `port`, `readiness`, `restart`, `logs`, and `backend`. Optional; absent on most machines. |
...
v1 is **config-declared only**. An agent-initiated service would need a worker tool, a new wire command, and an answer to who may declare a process that outlives the task requesting it (§13) — out of scope here.
