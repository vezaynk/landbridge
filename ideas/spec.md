...
| `landbridge-mcp` | Control plane HTTP today: both MCP surfaces, OAuth, `/runner`, dashboard, `Apply`. Splits into Core + LeadMCP + WorkerMCP ([`hub-and-write-queue.md`](hub-and-write-queue.md)). |
...
| `landbridge-hub` | Wake SSE (`hub_queue` tail) plus JSON twins. Same Postgres, own process. Internal; Bearer from MCP hosts / Blazor ([`hub-and-write-queue.md`](hub-and-write-queue.md)). |
...
**A Lead may act unattended, including closing sessions (§7, §9 check 4), without a human in the loop for each close.** This is not a new authority: the human remains the root, and delegation happens once, when the dashboard issues the factory token (§5 credential descent). What a Lead may never do is close a session its own worker produced — the doer/judge split holds structurally regardless of who is watching. Judgment calls a human must own are what `review` mode is for (§7).
...
There is no verifier credential class. Closing a session is a Lead-session or human act, never the session's own worker (§7, §9 check 4); an external automated close, if wanted, holds a Lead-class credential rather than a role of its own.
...
`hidden=true` is a default-off list filter. `send_input_request` unhides a stopped healthy row and `session/load`s. `hidden` plus `health=failed` still allows same-id retry (`session/new`). Successor-as-a-new-row is `fork_session` (#225).
...
`stop_session` closes the row (hide + `desired=on_disk`). It is a Lead or human close (**never** the session's own worker — doer/judge), not a grade of the work. Provenance (`lead-session` \| `human`) is recorded. Default wind-down is 5 minutes, then a kill. A report is mail, not a gate. More work on the same worker is `send_input_request` (unhides). Mid-exchange (a question or a live permission wait) is allowed.

**One targeting class.** Profile targeting routes a claimable session to any `ready` machine declaring its `profile`. Park, fail, and stop pin `preferred_machine` so `session/load` stays machine-local. If that machine is gone the row waits — there is no degrade cold-start. Successor-as-a-new-row is `fork_session` (#225).
...
**A registration names a port, not a process.** The listener may be a `start_process` child (machine-scoped, so it can outlive the registering task), a child of the worker's own shell, or something the operator runs under systemd or launchd. `landbridged` does not own ports and does not verify a dial against a declared fixture — a leftover `services[]` is refused at load (§10). Bind-first is the control that keeps the advertisement honest; a dead listener is a connection error, not a Landbridge refusal. The successful-impostor hazard (a dead service, something else now answering on that port) is real, and the answer is the same as it was before fixtures: bind a random port, register only after it answers, and treat a forward failure as a fact to report.
...
**Auth is per-mint, gated by default.** A mint may explicitly opt into a **public capability URL** — the unguessable label alone admits the request. Both public and gated default to a **2-hour idle TTL**; each admitted connection slides expiry by that window, so a preview under traffic does not die at wall-clock mint+TTL. Public remains time-boxed (a requested TTL is capped); gated is the default, and requires a §12 operator session.
...
**Lead:** `create_team` (mint a Team this factory token owns; the id is the capability) · `create_session` · `answer_input_request` · `stop_session` · `park_session` (deliberate release of a live ACP session) · `get_team_state` · `get_lead_inbox` · `watch_lead_inbox` · `list_profiles` (the routing read: which profiles exist and where they can run, §7; no Team id) · `bind_machine` · `unbind_machine` · `open_lead_forward` (§8.3 human path). Every Lead tool except `create_team` and `list_profiles` takes `teamId`. The Lead token is a factory, not a Team; an agent is given an id by a human or calls `create_team`. There is no `list_teams`.
...
- **No `claim_lead` / `release_lead` tools.** Lead identity *is* the credential: a human authenticates (§5 OAuth), the dashboard issues a factory token (Connect or a setup link), and `create_team` mints Teams that factory owns. There is no `list_teams`: an agent either minted the id or a human supplied it.
...
**Slash commands are a convenience layer, not the API.** Skills may mention `/landbridge-status` or `/landbridge-enroll` as triggers; none is registered as an MCP prompt on this build. Surfacing prompts as slash commands is client behaviour and not universal, so every command must map onto an independently-reachable surface — nothing may be reachable *only* through a prompt. That surface is a tool for agent actions (`get_team_state` with `teamId`), the §12 dashboard for human cross-Team/machine views, dashboard Connect for issuing a factory token, and the enrollment skill for joining a machine.
...
A leftover `services[]` block is refused at load. landbridged no longer supervises operator fixtures. Session-scoped long work is `start_process`; something that must survive a landbridged restart belongs to the machine's service manager.
...
**Names are machine-scoped and unique among live processes.** An exited process releases its name, so a retry is not blocked by its own corpse. The admission check and the insert are indivisible, or two concurrent calls could both see a name free and both take it. The **name arrives off the wire** here rather than from a file an operator wrote, which is the case the name validator exists for: it becomes a directory name, and the closed path construction depends on it being unable to steer one.

**Ports are out of scope for a process entirely.** This is a process manager: it keeps a process alive and reports how it ended. Reachability is a different noun with its own surface — §8.2 `register_service` — and keeping them apart means there is no overlap to reconcile and no half-registered state. A process declares no port, so it is `running` once its OS process is up (a crash on start is caught by the ordinary exit path, and a "still alive after N seconds" timer would be a heuristic dressed as a check), and a port clash between two processes is the agent's problem — consistent with there being no restarts. If a process happens to listen on something, that is the agent's business, exactly as if it had started the server from a shell. A forward dials whatever answers on the registered port; `landbridged` does not know which listener was intended.
...
**The four buckets are disjoint.** ACP `PromptResponse.usage` reports `inputTokens`, `outputTokens`, `cachedReadTokens`, and `cachedWriteTokens` as four columns that add up. Nothing here subtracts one from another.
...
1. SSH to the box.
2. Start the harness.
3. Connect to `landbridge-mcp`. Present a human-issued enrollment token; exchange for machine credentials. Declare name and OS. (As-built: **specs are not collected** — CPU/memory exist solely as live load on the heartbeat, never as declared capacity. There is no purpose or permission-level field.)
...
`discard` means removing this task's workspace instance, which is only safe *because* isolation is task-scoped. Under a shared checkout it would destroy a sibling's work. `discard` is deferred while a report is outstanding — deleting a workspace while the Lead may still be reading it turns a cancellation into a vanished pointer.
...
Park resume (above) recovers *one* task's transcript across a requeue. After `stop_session`, the same-id path is `send_input_request` (unhide + `session/load`). Successor-as-a-new-row is `fork_session` (#225).
...
Also, per machine, its **agent-started processes** (§10): name, state (`running` / `exited` / `stopped`), whether stdin is open, how long it has been up, last exit code, when it ended, and the session that started it (provenance, not ownership). This is the home for them because a process is machine-scoped and not Team-scoped, so it does not belong on a Team view. It arrives on the machine heartbeat and nowhere else; the plane stores what a machine last reported as last-value rows (`machine_processes`) and **interprets none of it** — it forms no opinion about whether a process is healthy. Live membership is `machines.last_spoke_at` within 90s; after that the box is absent from the view even if the rows remain. The `/runner` socket is still process-local and evaporates on Core restart; the columns do not. Human operator session only, consistent with the Machine Group view being a human surface and with machine enumeration remaining a non-goal for agents (§10 as-built). The Lead's `list_profiles` (§10) is carved out of that rule for **routing alone** and deliberately carries none of this: a process is per-machine state that no Team owns and that routing never consults, so knowing about it helps a Lead place exactly nothing. What a Lead needs a machine for is knowing where a profile can run, which is the whole of what that tool answers — "what is this machine running" stays here, with the person who can act on it.


**Process log *contents* are deliberately not served here.** `landbridged` captures them on the machine (below) and the operator or the starting agent reads the file there. Serving the bytes would be live tailing of a running process, which §16 open question 8 defers pending redaction — the terminal-task gate that makes transcript serving acceptable has no analogue for a process, which is never terminal. Status answers "is it up, did it die, why" without opening that question.

**The process list is an ordinary additive heartbeat field, and version skew is safe in both directions.** The heartbeat is deliberately outside §10's frozen command/event enum (`RunnerWire.Heartbeat` is in neither closed set, and the tripwire test that guards a vocabulary change asserts only those two sets), so adding to it is not a frozen-vocabulary change. Operationally that holds because the wire context sets `DefaultIgnoreCondition = WhenWritingNull` and **no type anywhere sets `UnmappedMemberHandling.Disallow`**: an older plane silently ignores a processes field it does not know, and a newer plane reading an older runner's heartbeat simply gets null. So a fleet upgrades incrementally with no lockstep plane/runner ordering. **That property is load-bearing and would disappear silently** if strict deserialization were ever turned on — anyone considering it should know it converts every future additive field, and every mixed-version fleet, into a breaking change.

**The one write on this view is Revoke machine** (§5, §13), and it is the exception that proves the read-only rule below: it does not tell a machine to do anything — it un-trusts one. Per machine, human-only for the same reason the view itself is (a machine belongs to no Team, so a Lead revoking one would be a Team un-trusting shared infrastructure; there is no MCP twin for the same reason), and same-origin like every other mutating form here — it is the most valuable POST on the surface to forge, since it needs nothing but a machine id and the ids are on a page the session can already read. One action takes away the box's credentials, its live `/runner` channel, and every worker instance token on it, requeueing what it held; the confirmation reports each. This is what makes §5's "un-trusting a machine must take seconds" reachable by an operator rather than a claim about a database. Enrolled-but-silent machines (`last_spoke_at` older than 90s, or never spoke) are absent from the view, so revoking one that has already dropped goes through the JSON twin with its id.


**Read-only otherwise, and the reason is restart-equals-reboot.** There is deliberately no dashboard start/stop/restart. A process has no config to re-read, so a "stop" button would need the plane to persist desired state on the machine — the invariant restart-equals-reboot exists to avoid (§10 runner restart). Cleanup is orchestration: a Lead sends a continuation and the resumed agent stops what it started. A `landbridged` restart kills the previous generation (the stray sweep) and nothing revives them. Always-on fixtures belong to systemd or launchd, not to this view.
...
The **name arrives off the wire**, so it faces the same validator a process name always does: it becomes a filesystem path segment, and the path construction is only closed because nothing untrusted can steer it.
...
**Name and OS are bound server-side at enrollment.** Clients report state; they do not re-declare their identity.
