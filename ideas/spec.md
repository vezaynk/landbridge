...
**A Lead may act unattended, including closing sessions (§7, §9 check 4), without a human in the loop for each close.** This is not a new authority: the human remains the root, and delegation happens once, at the lead-claim, where the human's session authority descends to the Lead (§5 credential descent). What a Lead may never do is close a session its own worker produced — the doer/judge split holds structurally regardless of who is watching. Judgment calls a human must own are what `review` mode is for (§7).
...
- **No `claim_lead` / `release_lead` tools.** A Lead is not an agent that claims a tool — Lead identity *is* the credential: a human authenticates (§5 OAuth), claims a Lead factory token through the lead-claim flow, and holds that token. `create_team` mints a Team that factory owns. Claiming/releasing the factory is still the credential lifecycle, not a task action. There is no `list_teams`: an agent either minted the id or a human supplied it.
