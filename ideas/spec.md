...
|---|---|
| **Harness** | Any ACP agent (native or adapter). `docketd` is the ACP client; `spawn` is the agent argv. |
...
| `blocked_on_input` → `submitted` | Lead or human | prose answer landed. A permission verdict returns in place to `working` and is a different tool. A prose answer writes a park record and redispatches (`session/load`). Does **not** touch the infrastructure counter |
...
**`stop` and `park_task` are ACP `session/cancel`, then a tree-kill at the TTL.** stdin is the JSON-RPC pipe, not a place to inject a wind-down turn. `preserve` is the plane's session id (`session/load` later), not the agent's cooperation. `ttl=0` is an immediate kill.

**The worker transport is ACP.** `docketd` is the Client; `spawn` is the Agent. Handshake is `initialize` → `session/new` (or `session/load`) → `session/prompt`. The plane MCP server is injected on `session/new` (`mcpServers`, HTTP + Bearer). Progress is `session/update` `tool_call`. Approvals are `session/request_permission`. There is no `events.mapping`, no `stdin: closed|deadman` profile key, and no `resume.args`. One ACP process per dispatch in this cut; multiplexed sessions on one host are protocol-legal and not implemented yet.
...
**The dead-man's switch is the ACP pipe.** `docketd` holds stdin for the worker's life. EOF means `docketd` is gone. An ACP agent that exits on stdin EOF tears down its tree. The stray sweep is the non-cooperative backstop. There is no `stdin: closed` profile: a harness that cannot share stdin with JSON-RPC is not a Docket worker.
...
**Permission waits are live ACP `session/request_permission`.** The process stays up. A prose question may end the turn (the worker called `request_input` and stopped). `endpoint_wait` is the park shape: the consumer is released; the plane wakes it when the service registers. There is no `--permission-prompt-tool` and no bypass flag.

**The permission flavor: an approval dialog answered by Docket instead of by nobody.** The ACP host — not the agent — raises `session/request_permission` when a tool call needs approval. The request carries the tool name plus the proposed arguments verbatim as untrusted agent-adjacent text. The task enters `blocked_on_input` so the inbox shows it. Wait TTL is off by default; `park_task` is how a forgotten request releases the machine.
...
**Routine approvals are answered by the ACP client** (`allow-once` for kinds the operator treats as routine). Ask-kinds go to the Lead. There is no harness bypass flag.

**Park writes a record: `{machine, working directory, harness session ref, attempt}`.** Redispatch prefers that machine *and that directory* and `session/load`s the stamped id. The resumed turn's prompt is the same worker brief; the *answer* is fetched on `get_task`. The predecessor is gone first — token revoked, `session/cancel` then kill — because two writers on one session corrupt it. Fresh MCP credentials ride `session/load`'s `mcpServers`. If the recorded machine is gone, redispatch cold-starts elsewhere from the workspace plus persisted notes.
