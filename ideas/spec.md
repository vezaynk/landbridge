...
**Stdin is a choice at start time.** By default a process gets a usable stdin pipe, which is what makes `write_process` work and what gives `stop_process` its graceful lever. `open_stdin: false` is for a program that would otherwise block forever reading input nobody will send: `docketd` redirects stdin and closes it at once, so the child's first read returns EOF rather than hanging. It is the portable choice — .NET has no cross-platform null-device handle, and leaving stdin un-redirected would hand the child whatever the daemon inherited, which is not a defined thing to give it. What it does **not** do is make stdin a terminal: a program that behaves differently because `isatty` is false behaves the same either way, since a real TTY needs a pty. So closing stdin fixes *blocking*, not *detection*, and it costs two things that are reported alongside the process so a cleanup agent can see them: `write_process` refuses with its own distinct cause, and stopping becomes the bounded wait and then a tree kill. **Choosing closed stdin is choosing a hard stop.**

**`write_process` is a pipe, not a TTY**, and the difference is documented where an agent will read it. It refuses outright against a process started with `open_stdin: false`, with a cause that says the pipe was never opened rather than that the name is wrong — the caller needs to learn this was a start-time choice, so it restarts the process differently instead of hunting for a typo. Programs that test whether stdin is a terminal behave differently on a pipe: no interactive prompts, block buffering instead of line buffering, and some refuse outright; a password prompt reading `/dev/tty` never sees these bytes, and a curses or full-screen program will not work at all. Success means the pipe accepted the bytes, never that the program understood them — the reply, if any, appears in the captured log, so the interaction idiom is *write, read the log, decide*. Payloads are capped at 16 KB per write, the same in-band prose discipline as elsewhere, UTF-8 only, newline-appended by default because line-oriented is the overwhelming case. There is deliberately no `signal_process`: signals do not port to Windows, stdin writes do.
