...
**Ingesting it into the plane is the next step and its shape is undecided.** Two paths: the plane hosts an OTLP receiver and points the harness at it (richest data, but harnesses would then talk to the plane outside the frozen wire above), or `docketd` parses usage off the harness's own stream and reports it as a new optional inbound wire event (keeps the transport monopoly and per-instance attribution, limited to what `docketd` can parse). Whichever lands, it feeds the §12 measured view; it does not become something enforced.
