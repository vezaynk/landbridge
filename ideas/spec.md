...
- **There is no release, and none is needed.** An earlier draft of this note carved out one — a task cancelled before it ever dispatched — and shipped a `ReleaseUndispatchedAsync` for it. That exception cannot arise: because the cap is committed *per dispatch*, inside the dispatch transaction (the bullet below), a task that never dispatched never charged the ceiling, and creation only asks whether headroom exists. So the carve-out described releasing a commitment that was never made, and the method it justified had no caller in the plane; both are removed. "Committed only rises" is therefore exact rather than approximate.
