...
**Worker:** `get_task` · `report_result` · `request_input` · `start_service` (§10) · `register_service` · `open_forward` · `open_preview` (§8.4)
...
- **`start_service` added (worker, §10).** A worker declares a long-lived background process — a dev server, a watcher, an indexer — and `docketd` supervises it as its own child, outside the worker's process tree, so it is not killed when the turn ends. It exists because agents that need this will otherwise discover the bad version of it (`setsid`, or scrubbing `DOCKET_*` to escape the reaper), which silently defeats the kill guarantee for everything else on the machine; the sanctioned path must therefore be at least as capable as the unsanctioned one. Deliberately **separate from `register_service`**: starting is a machine act, registering is a Team-visibility act, and fusing them would force every private helper to be advertised and would break "register only after the port answers." A port is **optional and a first-class absent** — plenty of useful background work listens on nothing.
...
### Agent-initiated services

**A service's lifetime is its declaring task's lifetime**, and that is what keeps the authority question answerable: a worker cannot create something outliving its own authority, because the service's life *is* the task's life and a task's life is already governed. No new §13 concept is required.

**The tagging is the policy.** `docketd` stamps the declaring task's id on the spawn, so the per-task exit sweep that already cleans up a worker's strays reaps the service too; a config-declared service deliberately carries no task id, which is precisely why it survives task exits. The two lifetimes differ in exactly one environment variable.

The teardown itself is `docketd`'s own bookkeeping — it stops a task's services when that task's process ends, and treats the stray sweep as the backstop for anything that escaped. Relying on the sweep alone would make this guarantee platform-dependent, since discovery finds nothing on Windows by design (containment there is the kill-on-close job, which fires on `docketd`'s death, not a task's exit).

**Restart-equals-reboot survives.** An agent-declared service has no config to re-read, so a naive design would force `docketd` to persist desired state — the invariant it refuses a dashboard stop button to protect. It does not need to: a restart reaps the previous generation, the tasks that machine held requeue, and the redispatched worker declares its service again. **The requeue path is the reconciliation.**

**What bounds it is the profile, not an allowlist of permitted commands.** `services.agent_initiated` is per-profile and off by default; enabling it is the machine owner's decision, the same shape as the open/strict archetypes. An allowlist would be the wrong control: a worker on an open profile can already start a dev server by hand, so restricting the sanctioned tool below its existing capability would only push agents back to the escape the worker skill forbids — and it would put domain knowledge in `docketd`. A strict profile with no shell cannot start a service either way and refuses honestly. `services.max_agent_initiated` is a separate, resource bound: the gate answers *may this task start services*, the cap answers *how many*, and a cap is needed because services are already-running load that back-pressure cannot gate the way it gates dispatch.

**Identity is `{task, name}`.** Two tasks may each hold a `dev`; what actually collides is the **port**, checked for uniqueness at admission exactly as config load checks it at rest — refuse-at-dial resolves a dial target by port, so a shared port would make that lookup answer for whichever it found first. The same task re-declaring a name it already holds **reclaims** it rather than being refused, so a retrying or redispatched worker is idempotent. A *different* task claiming a held port is refused, and told who holds it. Attaching to another task's service is deliberately not offered: the second caller's argv would be silently ignored, so it would receive a service configured as the first task specified while believing it had specified it — and it would make the lifetime multi-owner, reopening the authority question this design closes.

**A port-less service is a first-class shape**, not an edge case: a watcher, an indexer, a batch daemon. It is `running` once its process is up — the honest definition, since a "still alive after N seconds" timer is a heuristic dressed as a check, and a crash on start is already caught by the ordinary exit path. Such a service never participates in §8.2 registration or refuse-at-dial, because there is nothing to reach; status, logs, restart, and lifetime apply unchanged.

The **name arrives off the wire** here rather than from a file an operator wrote, which is the case the name validator exists for: it becomes a directory name, and the closed path construction depends on it being unable to steer one.

One honest limit: if the plane requeues a task for liveness loss while its worker process is still alive, the service outlives the task *as the plane understands it* until that process actually exits. This is the existing zombie-worker window rather than a new one, bounded by the same mechanics — the predecessor's token is already revoked — and it is recorded rather than solved.

Config-declared services remain, for operator-owned fixtures. Agent-initiated is the primary path.
...
**A worker may not create anything that outlives its own authority.** This is what makes `start_service` (§10) grantable at all: a service is bound to its declaring task, so its life is the task's life and the task's life is already governed — there is no new principal, no orphan process holding privileges nobody can revoke, and no state a restart must reconcile to work out who owns what. The corollary is that *attaching* to another task's service is refused: shared ownership would mean one task extending another's process beyond its own authority, which is exactly the thing this bound prevents.

Two consequences worth stating, because they are the difference between a bound and a slogan. Whether a task may start services at all is its **profile's** policy, declared by the machine's operator — a capability granted, never assumed, and a machine that grants nothing is unaffected by this feature existing. And the service's **name arrives off the wire**, so it faces the same validator a config-declared name does: it becomes a filesystem path segment, and the path construction is only closed because nothing untrusted can steer it.
