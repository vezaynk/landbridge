...
**One deliberate exception: a continuation inherits its predecessor's working directory** (§11). It has to — a harness session is resumable only from the directory that created it — and it is also what a continuation is *for*: carrying on the same work with the same context, where the workspace is that work. So two task ids can share one directory, by the Lead's own choice in asking for a continuation. Nothing else about them is shared: each keeps its own identity, credential, liveness, and transcript. The rule above still governs every *concurrent decomposition*, which is what it exists for — sibling tasks a Lead fans out must not collide, and they still cannot.
...
**`dispatch` names a task, never a path.** Two of its fields carry §11 resume: the opaque harness session ref to continue, and — for a continuation, which runs under a *new* task id — the task whose working directory that session lives in, because a harness session is directory-local as well as machine-local (Claude Code resumes only from the directory that created it). The second is a *task id* rather than a directory on purpose: `work_root` is machine-local runner config, and mapping a task to a directory under it is the runner's job, so the plane never learns or dictates a machine's filesystem layout. That is also why the park record's directory field never acquired a producer — the plane had nothing true to put in it. Both fields are absent on a first dispatch and on a same-task park-resume, where the runner's own directory for the task is already the right one.
...
Two smaller as-built notes on the same seam: `subagent-spawned` is in the wire vocabulary but has no producer, and `events.mapping` overrides the stdout stream's *property names* (`type_key`, `tool_use_block_type`, …) rather than mapping harness event names onto that vocabulary — with unrecognized keys falling back silently, so a fabricated mapping does not fail at load.
...
**And a third guard, which is what makes a continuation resume at all: it runs in the continued task's working directory.** A harness session is directory-local as well as machine-local, and a continuation is a *new task id* — so the runner's own `work_root/<task-id>` for it is a directory that has never held a session, and a resume aimed there fails outright rather than degrading to a cold start. The dispatch therefore names the directory-owning task (§10 — a task id, not a path) and `docketd` runs the harness there. The named task is resolved **transitively**, because resuming does not create a session per link: B continuing A resumes A's session *in A's directory*, so B never has a directory of its own, and C continuing B must still be sent to A's. Lineage alone cannot say that, so the directory-owning task is recorded as its own fact at creation. A `degrade` cold-start clears it along with the session ref and the preferred machine — that dispatch is starting a genuinely new session, which belongs in its own directory.

**So a continuation shares its predecessor's working directory, deliberately.** That is a real exception to §7's each-task-its-own-copy rule and it is the point rather than a leak: the continuation exists to keep working where the conversation left off, and the workspace *is* that work. Two consequences follow and are honoured. The dispatched task keeps its own identity for everything else — supervision, per-task liveness, stray reaping by task id, its transcript, and its own injected credential (which cannot take the borrowed directory's usual config filename, since the task that owns the directory may still be running). And `discard` becomes unsafe on a shared directory in exactly the way §11's cancellation section already warns about for a shared checkout, so a discard must not delete a directory the task merely borrowed; nothing enacts workspace discard today, which is the moment to get this right rather than after.
