...
`docketd --enroll <token> --control-url <plane>` performs the exchange at `POST /enroll`; the short-lived access token is re-minted at `POST /machine/refresh` (proactively at ~50% of its lifetime and reactively on a 401 reconnect), and credentials persist 0600 under the state dir (`--state-dir`, else `$XDG_STATE_HOME/docket`, else `~/.docket`).
