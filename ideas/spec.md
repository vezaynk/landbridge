...
**Lead:** `create_session` · `answer_input_request` · `stop_session` · `park_session` (deliberate release of a live ACP session) · `get_team_state` · `get_lead_inbox` · `watch_lead_inbox` · `list_profiles` (the routing read: which profiles exist and where they can run, §7) · `bind_machine` · `unbind_machine` · `open_lead_forward` (§8.3 human path)
