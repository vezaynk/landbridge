...
Stdio ↔ WebSocket pipe for ACP. `docketd` still owns a local process and
writes NDJSON to its stdin. This binary is that process, or the far-side
listener that wraps a real agent.

```
docket-acp-bridge listen [--bind 127.0.0.1:0] -- goose acp
docket-acp-bridge connect ws://127.0.0.1:PORT/acp
```
...
```json
"spawn": ["docket-acp-bridge", "connect", "ws://127.0.0.1:PORT/acp"]
```

Same machine, the session `cwd` docketd sends is a real path. The far-side
