...
| `Landbridge.Mcp`             | The ASP.NET **host** (`landbridge` + `landbridge-mcp`): the MCP tool surface agents connect to, the `/runner/*` machine endpoints, OAuth/enrollment/relay-validate HTTP endpoints, and the §12 web dashboard. One process, one Postgres, one Instance.                                                                  |
...
dotnet test tests/Landbridge.Core.Tests --no-build -c Release          # pure engine
dotnet test tests/Landbridge.Runner.Tests --no-build -c Release        # process supervision, per-OS reap
...
- **`.github/workflows/ci.yml`** (ubuntu only) builds the solution and runs the
  ...
  a `postgres:16` service container, each with its own database via `LANDBRIDGE_TEST_PG`
  (`landbridge_cp`, `landbridge_mcp`, `landbridge_meta`, `landbridge_multimachine`). GitHub-hosted
  ...
  that SIGKILLs real processes — it spawns the real plane, the real `landbridged` and real
...
- **`.github/workflows/publish-images.yml`** builds and pushes the `landbridge-mcp` and
  `landbridge-relay` runtime images to GHCR (linux/amd64 + linux/arm64), on a pushed
  `v*` tag or manual dispatch — never on a push to master, because `landbridge-meta`
...
**[`tools/Landbridge.HarnessBump`](tools/Landbridge.HarnessBump/README.md)** compares each pin against that
...
unit-tested (`dotnet test tests/Landbridge.HarnessBump.Tests`) rather than discovered on a live run
against real money.
...
- **Event sources beyond `terminal`.** `hooks` and `otel` parse but are wired to
  nothing, so they behave as `none`; `landbridged` warns loudly at startup for any
  profile declaring one. The subagent tree and `auth-failed` have no producer.
- The enrollment **conformance run** and `/landbridge-enroll` wizard do not exist; the
