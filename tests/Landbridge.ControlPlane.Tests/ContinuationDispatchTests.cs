...
/// tests pin what <see cref="SessionStore.DispatchNextAsync"/> does with it: prefer the
...
public sealed class ContinuationDispatchTests(PostgresFixture pg) : IAsyncLifetime
{
    ...
    public async Task Degrade_cold_starts_elsewhere_when_the_preferred_machine_is_gone()
    {
        ...
        var row = await v.Sessions.AsNoTracking().SingleAsync(t => t.Id == task.Value);
        ...
        var memoryLost = await v.SessionEvents.AsNoTracking()
            .SingleAsync(e => e.SessionId == task.Value && e.Kind == SessionEventRow.ContinuationMemoryLostKind);
    ...
    public async Task Pin_waits_in_submitted_and_dispatches_when_the_machine_returns()
    {
        ...
        Assert.IsType<StoreResult.NotFound>(
            await NewStore(db).DispatchNextAsync(Machine("m2"), WorkerInstanceId.New(), default, ["m2"]));
        await using (var v = pg.NewContext())
            Assert.Equal(SessionState.Submitted, (await v.Sessions.AsNoTracking().SingleAsync()).State);
    ...
    public async Task Get_team_state_renders_the_continues_lineage()
    {
        ...
        var summary = view.Sessions.Single(t => t.SessionId == task.Value);
        Assert.Equal(continued.Value, summary.ContinuesSessionId);
    }
    ...
    public async Task A_continuations_dispatch_carries_the_continued_tasks_directory()
    {
        ...
        Assert.Equal("sess-1", applied.HarnessSessionRef);
        Assert.Equal(continued, applied.WorkDirSession);
    }
    ...
    public async Task A_chain_of_continuations_all_name_the_root_tasks_directory()
    {
        ...
        Assert.Equal(a.Value, (await v.Sessions.AsNoTracking().SingleAsync(t => t.Id == b.Value)).WorkDirSessionId);
        var rowC = await v.Sessions.AsNoTracking().SingleAsync(t => t.Id == c.Value);
        Assert.Equal(a.Value, rowC.WorkDirSessionId);
        Assert.Equal(b.Value, rowC.ContinuesSessionId);
        // …and A itself, an ordinary task, claims nobody's directory.
        Assert.Null((await v.Sessions.AsNoTracking().SingleAsync(t => t.Id == a.Value)).WorkDirSessionId);
    }
    ...
    public async Task An_ordinary_tasks_dispatch_names_no_directory_even_when_resuming_its_own_park()
    {
        ...
        var first = Assert.IsType<StoreResult.Applied>(
            await NewStore(db).DispatchNextAsync(Machine("m1"), WorkerInstanceId.New(), default, ["m1"]));
        Assert.Null(first.WorkDirSession);
        ...
        Assert.Null(second.WorkDirSession);
    }
    ...
    public async Task A_degrade_cold_start_drops_the_session_but_keeps_the_directory()
    {
        ...
        Assert.Equal(continued, applied.WorkDirSession);     // but still the predecessor's dir
        await using var v = pg.NewContext();
        var row = await v.Sessions.AsNoTracking().SingleAsync(t => t.Id == task.Value);
        Assert.Equal(continued.Value, row.WorkDirSessionId);
