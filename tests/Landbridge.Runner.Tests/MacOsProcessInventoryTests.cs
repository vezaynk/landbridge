...
/// (libproc + <c>sysctl(KERN_PROCARGS2)</c>, unprivileged) and find the landbridge
...
public class MacOsProcessInventoryTests(Xunit.Abstractions.ITestOutputHelper output) : IDisposable
{
    ...
    public async Task Discovers_a_tagged_process_by_env_then_reaps_it()
    {
        ...
        var untagged = SpawnChild(machineId: null); // no LANDBRIDGE_MACHINE_ID — must never be listed
        ...
        var appeared = await TestKit.WaitUntilAsync(
            () =>
            {
                foreach (var p in inventory.ListLandbridgeProcesses())
                {
                    if (p.Pid == tagged.Id)
                    {
                        match = p;
        ...
        // The untagged child (no LANDBRIDGE_MACHINE_ID) must not be discovered at all.
        Assert.DoesNotContain(inventory.ListLandbridgeProcesses(), p => p.Pid == untagged.Id);
