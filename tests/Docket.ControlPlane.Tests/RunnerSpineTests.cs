...
public sealed class RunnerSpineTests(PostgresFixture pg) : IAsyncLifetime
{
    ...
    public async Task Started_event_refreshes_task_activity()
    {
        ...
        var sink = new RunnerEventSink(ScopeFactory(clock), registry, new ForwardWaiters(), new TranscriptWaiters(), new StartServiceRelay(registry), NullLogger<RunnerEventSink>.Instance);
