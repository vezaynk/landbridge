...
/// <c>Docket.Runner</c>'s <c>AcpClient</c>, and the reason the whole zero-cost E2E tier
...
/// docketd's half against a scripted peer, but both halves of that test are the same
...
/// docketd actually sends. It declares <c>loadSession</c> and <c>mcpCapabilities.http</c>
/// true, matching what every real agent measured on 2026-08-15 declares, so the capability
/// branches docketd takes here are the ones it takes in production.</para>
///
...
/// docketd translates the generated config → <c>mcpServers</c> on the wire → agent
...
/// consuming it would eat docketd's requests. EOF still ends the agent — it falls out of the
...
internal static class AcpAgentLoop
{
                ...
                switch (method)
                {
                    case "initialize" when hasId:
                        await RespondAsync(stdout, id, w =>
                        {
                            ...
                            // and therefore the branch docketd takes in production.
                            ...
                            w.WriteString("name", "docket-worker-harness");
                    ...
                    // additionally proves docketd took the resume branch, which the test
                    // asserts by the method name alone.
                    case "session/new" when hasId:
                    case "session/load" when hasId:
                        (url, authorization) = ReadDocketServer(root);
    ...
    /// moves docketd's progress clock — under ACP that is the <em>only</em> source of
        ...
        Func<string, string, string, CancellationToken, Task<int>> work, CancellationToken ct)
    {
        ...
        {
            // The spec's own word for a turn ended by session/cancel, and what docketd's
    ...
    /// long way rather than trusting position: this is the assertion that docketd sent
    /// something an agent can actually use.
    /// </summary>
    private static (string? Url, string? Authorization) ReadDocketServer(JsonElement root)
    {
