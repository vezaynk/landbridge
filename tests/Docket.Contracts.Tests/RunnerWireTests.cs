...
public class RunnerWireTests
{
    ...
    {
        Assert.Equal(
            new HashSet<string> { "dispatch", "stop", "kill", "open-forward", "read-transcript", "start-service" },
            new HashSet<string>(RunnerWire.Commands));
        Assert.Equal(
            new HashSet<string> { "started", "session-started", "alive", "tool-call", "subagent-spawned", "exited", "auth-failed", "forward-opened", "forward-closed", "rebooted", "transcript-chunk", "service-started" },
