...
activity telemetry to **your** OTLP collector, with the Landbridge task that caused
the spend stamped on every metric and event.

> **Visibility, not enforcement.** Nothing in Landbridge ingests, meters, or caps any
> of this: **the control plane never sees a token count.** Landbridge does not sit between
...
>
> **There is no spend limit anywhere in Landbridge.** A Team dollar ceiling existed until
...
Per profile, in landbridged's config, off by default:
...
**Why the harness flag lives in config.** landbridged contains no harness knowledge
(§10) — supporting a new harness is a config file, never a code change. So
landbridged sets only vendor-neutral OTel SDK variables and takes the harness's own
...
### What landbridged sets on the spawn

When `otel` is true **and** a destination resolves:

| Variable | Value |
|---|---|
| `OTEL_EXPORTER_OTLP_ENDPOINT` | `telemetry.endpoint`, else the one landbridged inherited |
...
| `OTEL_RESOURCE_ATTRIBUTES` | whatever was already there, plus `landbridge.session_id=<task>,landbridge.machine_id=<machine>` |
...
it. Neither map can set `LANDBRIDGE_MACHINE_ID`, `LANDBRIDGE_SESSION_ID`,
`LANDBRIDGE_WORKER_TOKEN`, or `LANDBRIDGE_TRACEPARENT`.
The one thing it cannot override is attribution: an `OTEL_RESOURCE_ATTRIBUTES`
you set becomes the *base* that `landbridge.session_id` is appended to, never a
replacement for it.

**Everything else you don't see here arrives by inheritance.** A worker is
spawned with `UseShellExecute=false`, so it gets a copy of landbridged's whole
environment. Headers, TLS material, compression, export intervals, the tracing
beta — set them once on landbridged and every worker has them. That is also why
`endpoint` is optional: a landbridged that already exports its own traces to a
...
- **`otel: false` (the default) sets nothing at all.** Not the exporters, not the
  ...
  it is opt-in per profile rather than something a Landbridge upgrade turns on.
- **Telemetry is never enabled without a destination.** `otel: true` with no
  ...
  to landbridged's stderr. An exporter aimed at nowhere costs a worker retry loops
  against a dead socket and buys no visibility.

## The dev loop: the Aspire dashboard is already a collector

`src/Landbridge.AppHost` runs an Aspire dashboard that is a real OTLP receiver, so
...
`src/Landbridge.AppHost/Properties/launchSettings.json`:
...
landbridged runs as an Aspire resource, so Aspire hands it the OTLP endpoint (and,
...
nothing in particular, so landbridged's port-based fallback would otherwise guess
...
landbridged and let every profile inherit it. Under the shipped service templates
that is `/etc/landbridged/landbridged.env` (`EnvironmentFile` in `deploy/landbridged.service`,
which already names this variable as typical contents) or the
`EnvironmentVariables` dict in `deploy/com.landbridge.landbridged.plist` — a machine-wide
destination that every profile can then opt into with `otel` alone.

Landbridge receives none of it and needs no network path to it: the worker talks to your
collector directly, and nothing about that traffic passes through the control plane.
There is still no OTLP receiver in Landbridge.

What Landbridge *does* now ingest arrives by a different route entirely — see
...
`landbridge.session_id` on each datapoint and not only in the OTLP resource block.
...
reason). Both variables can go in `telemetry.env`, or on landbridged for the whole
machine.

### Other harnesses may emit none of this

Everything in this section is Claude Code's. The names are `claude_code.*`, and all of
them are gated on `CLAUDE_CODE_ENABLE_TELEMETRY=1` — which landbridged carries as
...
vendor-neutral `OTEL_*` variables and appends `landbridge.session_id`, on a process that may
...
That is not a gap in what Landbridge measures, because the §12 measured view never depended on OTLP:
...
`landbridge_task_usage` has rows. See `ideas/skills/references/runner-config.md` for the usage mapping
...
derived by Landbridge, which is what the §12 label claims — but it is the harness's arithmetic over a
...
`landbridge.session_id` is that id. It rides `OTEL_RESOURCE_ATTRIBUTES`, so it lands on
every metric datapoint and every event the harness emits, and grouping by it in
your collector gives spend per Landbridge task — on a machine running several tasks
at once, in whatever mix of profiles.

`landbridge.machine_id` rides along too, because one Team's tasks span machines and
one machine serves many Teams.

**Task → Team is the plane's half, and it is not in this data.** A profile has no
Team, and `dispatch` carries no Team id, so landbridged cannot stamp one. The control
...
The task id also reaches a worker as `LANDBRIDGE_SESSION_ID` (§10, on every spawn,
...
`landbridged` reading the harness's own **stdout stream** — not by this OTLP path. The two
...
- **Tool accept/reject counts, lines-of-code, commits, pull requests, active time, and
  ...
  trace beta), so surfacing them needs Landbridge to host an OTLP receiver. That path was
  considered and rejected — see below.

### Why Landbridge does not host an OTLP receiver

It would deliver strictly more data. It was rejected because it does not work on the
machines Landbridge runs on:
...
- **`codex exec` ignores `OTEL_EXPORTER_OTLP_ENDPOINT`.** Its exporter comes from its own
  ...
  timeout constants. Since `landbridged` configures telemetry purely through spawn environment
...
- **Managed settings win.** On a machine whose MDM sets `OTEL_EXPORTER_OTLP_*`, Claude Code
  ...
  `landbridge.session_id` attribution the whole scheme depends on.
- **Release-build Codex exports to OpenAI by default.** Its built-in metrics exporter is
  Statsig (`ab.chatgpt.com`) unless its `config.toml` disables it. Nothing Landbridge sets
...
to hold it. Landbridge does not set them and a profile should not either.
...
- **Managed settings can override the endpoint.** Claude Code gives
  ...
  policy says, and `landbridge.session_id` rides along there instead. Verified on a
...
- **Cardinality is your collector's problem.** `landbridge.session_id` is unbounded over
...
- **Self-reported and machine-local.** A harness could report nothing, or report
  wrong. That is the standing reason nothing in Landbridge is enforced on these
