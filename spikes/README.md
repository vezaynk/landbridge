...
(matching docketd's own no-shell constraint, spec §10).

```sh
dotnet run --project spikes/Docket.Spikes -- s1   # resume-after-park
dotnet run --project spikes/Docket.Spikes -- s2   # stop as an injected turn
dotnet run --project spikes/Docket.Spikes -- s3   # hook events with task attribution
```

Model defaults to `haiku` to bound spend; override with `DOCKET_SPIKE_MODEL`.
