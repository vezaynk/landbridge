...
/// landbridged's own constraint (§10).
/// </summary>
public static class Program
{
    ...
    {
        Console.Error.WriteLine("usage: landbridge-spikes <s1|s2|s3>");
        ...
        Console.Error.WriteLine("  s3  per-tool-call hook events carrying LANDBRIDGE_SESSION_ID");
        Console.Error.WriteLine("env: LANDBRIDGE_SPIKE_MODEL (default: haiku)");
        return 2;
    }
}
...
internal static class SpikeEnv
{
    public static string Model =>
        Environment.GetEnvironmentVariable("LANDBRIDGE_SPIKE_MODEL") ?? "haiku";

    /// <summary>spikes/out/&lt;name&gt;, located relative to this project.</summary>
    public static string OutDir(string name)
    {
        // bin/Debug/net10.0 → Landbridge.Spikes → spikes
